...
def event_driven_signals():
    ...
    # Then route the new theses to CURRENT autonomous execution — the
    # Virtual CEX. This used to fire jobs.execute_signals, the Alpaca
    # submitter, so a thesis generated by an event went straight at an
    # external broker path in a VIRTUAL_ONLY platform.
    if job_status['paper']['status'] != 'running':
        from jobs.paper_trading import run as paper_run
        timer = threading.Timer(15.0, make_job_runner('paper', paper_run))
        timer.daemon = True
        timer.start()


# ── THE EXTERNAL BROKER CONNECTOR, OFF BY DEFAULT ───────────────────────
# JARVIS is VIRTUAL_ONLY. Its autonomous execution is the Virtual CEX
# (jobs.paper_trading) and the Virtual DEX (lib.dex_autotrade); its risk is
# the aggregate of those two books.
#
# Three jobs predate that architecture and still described the desk as
# live-first: `execute` submitted orders to Alpaca, `positions` managed
# Alpaca positions, and `guardian` read the Alpaca account and called
# itself THE portfolio guardian. An external broker account may legitimately
# be DATA, REFERENCE or GROUND TRUTH — it must not silently define the
# desk's current training positions, training risk or autonomous execution.
#
# They are quarantined behind this flag rather than deleted: the defensive
# logic inside them is worth keeping for the day live execution returns.
def external_connector_enabled() -> bool:
    """True only when the operator has explicitly opted in."""
    return os.getenv("JARVIS_ENABLE_EXTERNAL_BROKER_CONNECTOR", "").lower() in (
        "1", "true", "yes")


def external_alpaca_guardian():
    """
    EXTERNAL ACCOUNT guardian — opt-in, and NOT the desk's own risk view.

    Renamed from `portfolio_guardian`. It reads the Alpaca account, which
    under VIRTUAL_ONLY is an external reference book rather than the
    training book, so presenting it as the portfolio guardian misdescribed
    whose risk was being guarded.
...
def create_scheduler(*, economic: bool = True) -> BackgroundScheduler:
    ...
    # ── EXTERNAL BROKER CONNECTOR — opt-in only ───────────────────────────
    # `execute` submits to Alpaca and `guardian` reads the Alpaca account.
    # Neither describes current autonomous execution or current training
    # risk under VIRTUAL_ONLY, so neither is scheduled by default. The code
    # is retained; only its automatic scheduling is withdrawn.
    if external_connector_enabled():
        logger.warning(
            "[Scheduler] EXTERNAL BROKER CONNECTOR ENABLED — Alpaca execute "
            "and external account guardian are scheduled. These are NOT the "
            "desk's virtual training path.")
        sched.add_job(make_job_runner('execute', execute_run),
                      'interval', minutes=30, id='execute',
                      next_run_time=now + timedelta(minutes=4))
        sched.add_job(make_job_runner('guardian', external_alpaca_guardian),
    ...
    # `positions` (Alpaca position management) is registered above, and only
    # when the external broker connector is explicitly enabled.
